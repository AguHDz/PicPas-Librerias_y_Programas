...
*
*  Compilador XC8 v.1.35 (http://www.microchip.com)
*  VERSION REGISTRADA DE PAGO. La versión gratis no consigue
*  compilar este código en menos de 1K del microcontrolador.
*
...
void LCD_init(void)
{
    ...
    __delay_ms(200);   // Espera para asegurar tensión estable tras inicio.
    LCD_RS = LOW_ST;
    LCD_EN = LOW_ST;
...
uint8_t cicloTimeSet(uint8_t limInf, uint8_t limSup, uint8_t lcdX, uint8_t lcdY, uint8_t editDato)
{
    while((P_INC && P_DEC)==LOW_ST)  // Si se pulsa INC o DEC.
    {
        ...
        __delay_ms(TIEMPO_REPETICION);   // Espera el tiempo de autorepetición de la tecla pulsada.
    }

    if(P_SET==LOW_ST)                // Si se pulsa SET.
    {
        editMenuState++;
        while(P_SET==LOW_ST) __delay_ms(TIEMPO_ANTIREBOTE);  // Espera antirebote mecánico del pulsador.
