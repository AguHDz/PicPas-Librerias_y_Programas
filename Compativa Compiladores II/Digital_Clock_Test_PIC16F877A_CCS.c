/*
*  (C) AguHDz 03-OCT-2017
*  Ultima Actualizacion: 03-NOV-2017
*
*  Compilador CCS C v.5.0.74 (http://www.ccsinfo.com/)
*
*  Microcontrolador: PIC16F877A
*
*  RELOJ DIGITAL (TEST COMPARACION COMPILADORES)
*  =============================================
*  Este proyecto es una demostración del uso del compilador CCS S con
...
#FUSES NOPROTECT, NODEBUG, NOWRT, NOCPD, NOLVP, BROWNOUT, PUT, NOWDT, HS
...
#define SOUT                input_state(PIN_A3)         // Pin que lee la salida SOUT
//
// __Comunicación I2C__________________________________________________________
#define SDA                 PIN_B0         // Pin SDA del bus I2C
#define SCL                 PIN_B1         // Pin SCL del bus I2C
#define SDA_HIGH            output_high(SDA)     // SDA nivel alto. (HIGH)
...
#define P_DEC               input_state(PIN_A1)    // Pulsador DEC
#define P_SET               input_state(PIN_A2)    // Pulsador SET
...
#define LCD_DATA_4          PIN_B4         // Pines de datos
#define LCD_DATA_5          PIN_B5
#define LCD_DATA_6          PIN_B6
#define LCD_DATA_7          PIN_B7
#define LCD_RS              PIN_B2         // Pin RS
#define LCD_EN              PIN_B3         // Pin Enable
#define LCD_RS_HIGH         input_state(LCD_RS, HIGH_ST)
#define LCD_RS_LOW          input_state(LCD_RS, LOW_ST)
#define LCD_EN_HIGH         input_state(LCD_EN, HIGH_ST)
#define LCD_EN_LOW          input_state(LCD_EN, LOW_ST)
// Valores de RS.
...
{
    output_bit(LCD_DATA_4, (date & 0x10) == 0x10);
    output_bit(LCD_DATA_5, (date & 0x20) == 0x20);
    output_bit(LCD_DATA_6, (date & 0x40) == 0x40);
    output_bit(LCD_DATA_7, (date & 0x80) == 0x80);
    output_bit(LCD_EN, HIGH_ST);
    delay_ms(2);
...
{
    output_bit(LCD_RS, LCD_CmdMode);
    LCD_send(comm);
}

void LCD_putChar(uint8_t date)
{
    output_bit(LCD_RS, LCD_CharMode);
    LCD_send(date);
}
...
*****************************************************************************/
void I2C__start(void)     // START function for communicate I2C
{
    SDA_HIGH;
    SCL_HIGH;
    SDA_LOW;
...
void I2C__stop(void)     // STOP function for communicate I2C
{
    SDA_LOW;
    SCL_HIGH;
    SDA_HIGH;
...
bool I2C__writeByte(uint8_t dato)     // Send data to I2C
{
    uint8_t LoopCounter;
    bool ACKbit;
    ...
    {
        output_bit(SDA, (dato & 0x80) == 0x80);    // SDA = bit de más peso del valor dato.
        SCL_HIGH;
        dato<<=1;
        SCL_LOW;
    }
...
void DS1307_timeRead(void)
{
    ...
    I2C__writeByte(0x00);  // Primera dirección a leer/escribir.
    I2C__start();          // Reinicia comunicación I2C.
    I2C__writeByte(0xD1);  // DS1307 en Modo Escritura.
    DS1307_Segundo   = BCDToDecimal(I2C__readByte(1)); // ASK = 1
    DS1307_Minuto    = BCDToDecimal(I2C__readByte(1));
    DS1307_Hora      = BCDToDecimal(I2C__readByte(1));
    DS1307_DiaSemana = I2C__readByte(1);  // Valor 1...7 (igual en decimal que en BCD)
    DS1307_Dia       = BCDToDecimal(I2C__readByte(1));
    DS1307_Mes       = BCDToDecimal(I2C__readByte(1));
    DS1307_Ano       = BCDToDecimal(I2C__readByte(0)); // ASK = 0
...
void DS1307_timeWrite(void)
{
    ...
    I2C__writeByte(0xD0);  // Dirección I2C del DS1307.
    I2C__writeByte(0x00);  // Primera dirección a leer/escribir.
    I2C__writeByte(0);     // Siempre que se ajusta la fecha y hora los Segundos=0.
    I2C__writeByte(decimalToBCD(DS1307_Minuto));
    I2C__writeByte(decimalToBCD(DS1307_Hora));
    I2C__writeByte(DS1307_DiaSemana);  // Valor 1...7 (igual en decimal que en BCD)
    I2C__writeByte(decimalToBCD(DS1307_Dia));
    I2C__writeByte(decimalToBCD(DS1307_Mes));
    I2C__writeByte(decimalToBCD(DS1307_Ano));
    I2C__stop();
}
...
void setup(void)
{
    ...
    I2C__start();                 // Inicia comunicación I2C
    I2C__writeByte(0xD0);         // Dirección I2C del DS1307.
    I2C__writeByte(0x07);         // Escribe en la dirección 07h.
    I2C__writeByte(DS1307_CONF);  // Configura 1 Hz en salida SOUT del DS1307.
    I2C__stop();
